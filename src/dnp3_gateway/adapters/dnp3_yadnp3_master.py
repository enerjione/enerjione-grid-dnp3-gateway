...
# --------------------------------------
# Sondalar TESHIS icindir, saglik olcutu DEGIL (bkz. network_probe).
# Smart cihazda sonda YALNIZCA beklenen rapor penceresi kacirildiginda
# (yani `late` iken) calisir: uyuyan bir modeme ping atmak hem anlamsizdir
# hem de Smart modun guc/oturum davranisini etkileyebilir.
#
# 300 sn: bir cihazin tanilamasi 5 dakikada bir yenilenir. Daha sik olmasi
# operatore yeni bilgi vermez, 600 cihazda ise gereksiz yuk uretir.
_PROBE_MIN_INTERVAL_SEC = 300.0

# DIAL-IN (ZAMANLANMIS RAPOR) BEKLENTISI
# --------------------------------------
# Horstmann Dial-In araligi cihazin NE ZAMAN haber vermesi BEKLENDIGINI
# soyler. `smart_max_silence_sec` ise "artik gercekten kopuk" esigidir ve
# GERIYE UYUM icin OTORITE OLARAK KALIR.
#
#     son gecerli DNP3 kaniti + dial_in_interval  = beklenen rapor ani
#
#     beklenen rapor anindan ONCE   -> smart_idle (SAGLIKLI)
#     sonra, max_silence'tan ONCE   -> late      (DEGRADED, comm_lost YOK)
#     max_silence sonrasi           -> lost      (comm_lost, TEK KEZ)
#
# `dial_in_interval_min` YOKSA `late` durumu HIC uretilmez ve davranis
# 1.13.0 ile birebir aynidir.
#
# Formul UYDURULMADI: yuzde bazli bir tolerans EKLENMEZ. Iki esik de
# yapilandirmadan gelir; gateway ikisinin arasini "gecikmis" olarak
# adlandirmaktan baska bir sey yapmaz.
...
class _DeviceCache:
    ...
    def __init__(self) -> None:
        ...
        #: KANIT NESLI — her gecerli DNP3 kanitinda (olcum ya da IIN/gorev
        #: basarisi) BIR ARTAR. Recovery sinirinda zaman damgasi YERINE bu
        #: kullanilir: `time.monotonic()` Windows'ta ~15.6 ms cozunurluklu ve
        #: `fail_recovery` ile hemen ardindan gelen gercek bir kanit AYNI
        #: tick'e dusebiliyor — o durumda zaman karsilastirmasi yeni kaniti
        #: YOK SAYARDI. Sayac saat cozunurlugunden BAGIMSIZDIR.
        self._evidence_seq: int = 0
        #: Son BASARISIZ recovery anindaki kanit nesli. Bir sonraki relink
        #: icin SINIR: ayni eski kare recovery'yi bir daha tetikleyemez.
        self._recovery_failed_seq: int | None = None
        #: Smart uyanma penceresinde comm_lost ERTELEMESININ baslangici.
        #: Link FLAP ederse `recovery_age()` her `OnOpen`da sifirlanir;
        #: erteleme ona baglansaydi bozuk bir cihaz SONSUZA KADAR gizlenirdi.
        #: Bu damga YALNIZCA gercek `online` gecisinde sifirlanir.
        self._comm_lost_erteleme_at: float = 0.0
        ...
        *,
        ...
        with self._lock:
            ...
            if self._state == "recovering" and now >= self._recovery_anchor_at:
                ...
                # Cihaz GERCEKTEN konustu: erteleme hakki yenilenir.
                self._comm_lost_erteleme_at = 0.0
    ...
    def g110_gerekli(self, *, max_deneme: int | None = None) -> bool:
        ...
        `max_deneme`: oturum basina AZAMI deneme. SMART politikada `1` verilir
        (bkz. cagiran taraf): orada 15/30/60/120/240 saniyelik backoff
        yeniden denemeleri, sessizlik penceresinin TAM ORTASINA dusen
        TEKRARLAYAN uygulama istekleri uretir ve Horstmann'in hareketsizlik
        sayacini sifirlar. Tek atislik deneme "acilis isi"dir ve sozlesmece
        serbesttir; tekrarlayan deneme DEGILDIR.
        """
        with self._lock:
            ...
            if simdi < self._g110_sonraki_at:
                return False
            tavan = _G110_MAX_DENEME if max_deneme is None else max(0, int(max_deneme))
            if self._g110_deneme >= tavan:
                return False
            self._g110_deneme += 1
    ...
    def begin_recovery(self) -> None:
        ...
        with self._lock:
            ...
            self._pending_recovery_publish = False
            # Yeni deneme basladi: eski basarisizlik siniri artik gecerli degil.
            self._recovery_failed_seq = None
    ...
    def fail_recovery(self) -> None:
        ...
        O ANDAKI KANIT NESLI KAYDEDILIR (`_recovery_failed_seq`). Bu, bir
        sonraki relink denemesinin SINIRIDIR: ayni ESKI kare bir daha
        recovery tetikleyemez.
        """
        with self._lock:
            if self._state == "recovering":
                self._state = "lost"
                ...
                self._recovery_anchor_at = 0.0
                self._pending_recovery_publish = False
                self._recovery_failed_seq = self._evidence_seq

    def comm_lost_ertelenebilir(self, grace_sec: float) -> bool:
        """Smart uyanma pazarligi sirasinda comm_lost ERTELENSIN mi?

        ILK cagrida sayac baslar; `grace_sec` dolunca KALICI olarak False
        doner ve comm_lost YAYINLANIR.

        NEDEN `recovery_age()` YETMEZ: link FLAP ederse (cihaz baglanip
        hemen kopuyorsa) her `OnOpen` yeni bir `begin_recovery()` uretir ve
        `recovery_age()` sifirlanir. Erteleme ona baglansaydi GERCEKTEN
        bozuk bir cihaz sonsuza kadar "saglikli" gorunurdu — smart
        politikanin en tehlikeli sessiz basarisizlik modu tam da budur.

        Bu damga yalnizca cihaz GERCEKTEN `online` olunca sifirlanir.
        """
        now = time.monotonic()
        with self._lock:
            if not self._comm_lost_erteleme_at:
                self._comm_lost_erteleme_at = now
                return True
            return (now - self._comm_lost_erteleme_at) <= max(0.0, grace_sec)

    def relink_izinli(self) -> bool:
        """`lost` -> `recovering` gecisi icin YENI kanit var mi?

        SAHA HATASI (2026-08-20): `lost` durumundan cikis kosulu yalnizca
        "kanit bayat degil" idi. Kanit yasi esigin (120sn) altinda kaldigi
        surece ESKI bir kare relink'i TEKRAR TEKRAR tetikliyordu:

            lost -> relink -> recovering -> 15sn grace -> timeout -> lost
                 -> relink (AYNI eski kare) -> ...

        Sahada tam olarak ~15 saniyelik bir salinim gorulduu. Recovery
        hicbir zaman tamamlanamiyordu cunku tamamlanma YENI veri ister ve
        yeni veri gelmiyordu.

        Cozum MONOTONIK KANIT SINIRI: bir recovery basarisiz olduktan sonra
        yeni bir deneme icin o BASARISIZLIKTAN SONRA gelmis bir kanit (ya da
        yeni bir link acilisi) SART.

        SINIR ZAMAN DAMGASI DEGIL SAYACTIR: `time.monotonic()` Windows'ta
        ~15.6 ms cozunurluklu ve `fail_recovery` ile hemen ardindan gelen
        GERCEK bir kanit AYNI tick'e dusebiliyor — zaman karsilastirmasi o
        kaniti YOK SAYAR ve cihazi gereksizce `lost` tutardi. Sayac saat
        cozunurlugunden BAGIMSIZDIR.
        """
        with self._lock:
            if self._recovery_failed_seq is None:
                return True
            return self._evidence_seq > self._recovery_failed_seq
    ...
    def enter_smart_idle(self, *, anchor_wall: float | None = None) -> bool:
        """`smart_idle`e ELLE gir. Donen: yeni girildi mi (idempotent).

        LISTENING SMART ICIN ZORUNLU: orada baglantiyi GATEWAY acar ve cihaz
        uykudayken TCP baglantisi KURULAMAZ. `OnOpen`/`OnClose` hic
        tetiklenmez, dolayisiyla `set_connected(False)` yolundan idle'a
        girilemez. Cihaz "uyuyor" kabul edilir ve sessizlik/Dial-In denetimi
        onu zamaninda `lost`a dusurur.

        `anchor_wall`: hic gecerli temas gorulmemisse sessizlik sayacinin
        capasi (tipik olarak master'in kurulus ani). Boylece hic konusmamis
        bir cihaz da SONSUZA KADAR "saglikli uyuyor" gorunmez.
        """
        with self._lock:
            if self._smart_idle_since != 0.0:
                return False
            self._smart_idle_since = time.monotonic()
            self._smart_idle_since_wall = time.time()
            # DURUMU DA YAZ. `set_connected(False)` yolunda bu satirin
            # karsiligi zaten var; buraya konmazsa cihaz `_smart_idle_since`
            # dolu ama `_state="lost"` kalir — yani icerideki mantik "uyuyor"
            # derken /health ve SCADA "kopuk" gorurdu.
            self._state = "smart_idle"
            if not self._last_valid_contact and anchor_wall:
                # Kanit YOK: sayaci baslatmak icin capa kullanilir. Bu bir
                # "temas" DEGILDIR; yalnizca denetimin baslangic noktasidir.
                self._last_valid_contact = time.monotonic()
                self._last_valid_contact_wall = float(anchor_wall)
            return True
...
def _make_channel_listener(device_code: str, durum_yaz: Any) -> Any:
    """yadnp3 KANAL durumu dinleyicisi — TCP teshisinin PASIF kaynagi.

    NEDEN BU, HAM SOKET DEGIL (PR #32 review, madde 2)
    --------------------------------------------------
    Cihazin DNP3 portuna tanilama amacli ham bir `socket.connect()` acmak,
    uretim DNP3 kanaliyla YARISAN ikinci bir baglanti demektir. Smart moddaki
    bir Horstmann yalnizca sinirli bir pencere boyunca uyanik kalir; ham soket
    yarisi kazanip once baglanir, hicbir DNP3 trafigi uretmeden kapanir ve
    GERCEK oturumu engeller. Olcum araci olctugu sistemi bozmus olur.

    Buradaki dinleyici tamamen PASIFTIR: ek soket YOK, ek trafik YOK, yaris
    YOK. Kanal zaten baglanmaya calisiyor; biz yalnizca ONUN durumunu
    okuyoruz.

    OLCULEN DURUM DIZISI (yadnp3 3.2.1.1, gercek soketle dogrulandi):

        OPENING -> OPEN -> CLOSED -> OPENING -> OPEN

    KUTUPHANE "REDDEDILDI" ile "PAKET DUSTU" AYRIMINI VERMEZ. O ayrim
    UYDURULMAZ; `CLOSED` icin `unknown` raporlanir.

    Bu callback opendnp3'un IO thread'inden gelir: hicbir istisna disari
    sizmamali ve hicbir is BLOKLAMAMALI (tek satir atama).
    """
    if not _YADNP3_AVAILABLE:
        raise Yadnp3AdapterError(f"yadnp3 yuklu degil: {_YADNP3_IMPORT_ERROR}")

    class _ChannelListener(opendnp3.IChannelListener):  # type: ignore[misc]
        def __init__(self) -> None:
            super().__init__()

        def OnStateChange(self, state):  # noqa: N802
            try:
                durum_yaz(kanal_durumu_tokeni(state))
            except Exception:  # noqa: BLE001
                logger.debug("channel_state_error device=%s", device_code, exc_info=True)

    return _ChannelListener()


def kanal_durumu_tokeni(state: Any) -> str:
    """opendnp3 `ChannelState` -> `tcp_probe_status` tokeni.

    DURUSTLUK KURALI: kutuphanenin vermedigi bir ayrim URETILMEZ.
    `CLOSED`/`SHUTDOWN` "reddedildi" mi "paket dustu" mu, ya da cihaz mesru
    olarak uykuda mi — bilinmiyor, dolayisiyla `unknown`.
    """
    ad = getattr(state, "name", None) or str(state).rsplit(".", 1)[-1]
    if ad == "OPEN":
        return network_probe.TCP_OPEN
    if ad == "OPENING":
        return network_probe.TCP_CONNECTING
    return network_probe.TCP_UNKNOWN
...
class _OturumDurumu:
    """Master'in OTURUM/POLITIKA calisma-zamani alanlari — TEK KAYNAK.

    NEDEN AYRI BIR SINIF: bu alanlarin varsayilanlari testlerdeki taklit
    master siniflarinda ELLE tekrarlaniyordu ve her yeni alan eklendiginde
    taklitler sessizce SAPIYORDU (`AttributeError`, ya da daha kotusu:
    gercekte olmayan bir davranisi dogrulayan yesil test). Taklitler artik bu
    sinifi miras alip `_oturum_durumunu_kur` cagirir; alan eklemek tek yerde
    yapilir ve sapma IMKANSIZ hale gelir.

    `_ManagedMaster` disinda kalan hicbir sey burada YOKTUR: kanal, master
    nesnesi, tarama tutamaklari gercek sinifin isidir.
    """

    def _oturum_durumunu_kur(self, *, session_policy: str, known_mode: str | None) -> None:
        ...
        #       saniyeye ulasmazsa modem kapanamaz.
        #
        ...
        #: `auto` sinifladirmasinin BAGLANTILI gectigi sure (monotonic).
        self.auto_connected_since: float = 0.0
        #: Sinifladirma penceresi dolup `continuous`a dusuldu mu.
        self.auto_fallback: bool = False
        #: Bu OTURUMDA siniflandirma integrity poll'u gonderildi mi
        #: (yalnizca `listening`). Baglanti kopunca sifirlanir; boylece her
        #: yeni oturumda EN FAZLA BIR kez sorulur.
        self.auto_siniflama_pollu: bool = False
        ...
        #: Master'in kuruldugu duvar saati. LISTENING+SMART'ta ilk gecerli
        #: temas HENUZ olmadan idle'a girilebildigi icin "sessizlik ne zaman
        #: baslamis sayilsin" sorusunun cevabi budur; olmasa sessizlik denetimi
        #: hicbir zaman baslamaz ve gercekten olu bir cihaz SONSUZA KADAR
        #: "uyuyor" gorunurdu.
        self.olusturuldu_wall: float = time.time()
        #: Beklenen Dial-In raporu kacirildi mi (LATE / DEGRADED).
        #: `lost` DEGILDIR ve comm_lost URETMEZ.
        self.rapor_gecikti: bool = False
        #: Aktif tanilama sondasi — son calistirma + son sonuclar.
        #: SALT TESHIS; saglik karari VERMEZLER.
        self.sonda_son_at: float = 0.0
        self.sonda_son_wall: float | None = None
        self.ip_probe: str = network_probe.IP_UNKNOWN
        #: TCP teshisi yadnp3 KANAL DURUMUNDAN gelir; ham soket ACILMAZ.
        #: IO thread'i yazar, poll/health thread'i okur -> lock'lu.
        self.tcp_probe: str = network_probe.TCP_UNKNOWN
        self._kanal_lock = threading.Lock()

    def _kanal_durumu_yaz(self, token: str) -> None:
        """Kanal durumu tokenini sakla. opendnp3 IO THREAD'inden cagrilir.

        Tek atama + lock; hicbir is BLOKLAMAZ. Bu callback'te log basmak ya
        da karar uretmek IO thread'ini bekletmek olurdu.
        """
        with self._kanal_lock:
            self.tcp_probe = token

    def kanal_durumu(self) -> str:
        with self._kanal_lock:
            return self.tcp_probe


class _ManagedMaster(_OturumDurumu):
    ...
    def __init__(
        ...
        *,
        ...
        # KANAL DURUMU DINLEYICISI — TCP teshisinin PASIF kaynagi.
        #
        # REFERANS BU NESNEDE TUTULMAK ZORUNDA: pybind11 kendisine gecirilen
        # Python nesnesine sahiplik ALMAZ. Gecici bir nesne verilirse (orn.
        # `AddTCPClient(..., _make_channel_listener(...))`) cop toplayici onu
        # toplar ve callback'ler SESSIZCE hic gelmez — OLCULDU.
        self._channel_listener = _make_channel_listener(device.code, self._kanal_durumu_yaz)
        ...
        # ---- LINK KEEPALIVE — SMART'ta KAPATILIR --------------------------
        #
        # OLCULDU (yadnp3 3.2.1.1): `cfg.link.KeepAliveTimeout` varsayilani
        # 60000 ms'dir ve gateway onu HIC ayarlamiyordu. Tamamen sessiz bir
        # master bile 60. saniyede 10 baytlik bir LINK_STATUS gonderiyor.
        #
        # Bu, Smart Mode icin OLDURUCU: Horstmann dokumantasyonu hareketsizlik
        # sayacinin "HER TCP/DNP trafigiyle" sifirlandigini soyler. 60 saniyede
        # bir keepalive, 600 saniyelik oturum sayacini SONSUZA KADAR sifirlar —
        # yani taramalar kapatilsa BILE modem hicbir zaman uyuyamaz. Saha
        # yakalamasi 17 saniyelikti ve bu cerceveyi GORMEDI; config duzeltilip
        # taramalar sustuktan sonra ISIRACAK olan hata buydu.
        #
        # `TimeDuration.Max()` keepalive'i fiilen devre disi birakir (olculdu:
        # 70 saniyede SIFIR cerceve).
        #
        # CONTINUOUS'TA DOKUNULMAZ: orada keepalive olu link tespiti icin
        # degerlidir ve zaten 5 saniyede bir tarama gittigi icin hicbir zaman
        # tetiklenmez. Duzeltme CIHAZ BAZINDADIR (bkz. review §11).
        if self.session_policy == "smart":
            try:
                cfg.link.KeepAliveTimeout = opendnp3.TimeDuration.Max()
            except Exception:  # noqa: BLE001
                # Binding bu alani sunmuyorsa Smart sessizligi TAM olmaz;
                # sessizce gecmek yerine operatore soyle.
                logger.warning(
                    "yadnp3_keepalive_disable_failed device=%s — link keepalive "
                    "kapatilamadi; Horstmann hareketsizlik sayaci 60sn'de bir "
                    "sifirlanabilir ve modem uyuyamayabilir",
                    device.code,
                )
        ...
        # scan'den kacinir.
        self._master.Enable()
        # SAHA TESHIS SATIRI — tcpdump ACMADAN cevaplanmasi gereken soru:
        # "bu cihaz kazara `continuous` mu kosuyor?"
        #
        # 2026-08-20 saha olayinda bu ayrimi gormek icin paket yakalamak
        # gerekti. Satir `periodic_scans` bayragini ACIKCA basar: `true`
        # gorulduyse cihaz 5 saniyede bir Class 1/2/3 istegi gonderiyor
        # demektir ve Horstmann'in 600sn hareketsizlik sayaci ASLA dolamaz.
        #
        # `configured` ve `effective` AYRI basilir: `auto` cihazda ikisi
        # farkli olabilir ve teshis tam olarak o farktadir. Token/kimlik
        # bilgisi BASILMAZ.
        periyodik = self.session_policy != "smart"
        logger.info(
            "yadnp3_master_enabled device=%s mode=%s endpoint=%s remote=%s local=%s "
            "ip_endpoint_type=%s configured_policy=%s effective_policy=%s "
            "operation_mode=%s periodic_scans=%s event_scan=%s baseline_scan=%s",
            device.code,
            channel_mode,
            ...
            local_address,
            endpoint_type,
            self.configured_session_policy,
            self.session_policy,
            self.operation_mode,
            "true" if periyodik else "false",
            f"{self._scan_interval_sec}s" if periyodik else "-",
            f"{self._baseline_interval_sec}s" if periyodik else "-",
        )

    @staticmethod
    def _listening_retry(device: DeviceConfig) -> Any:
        """`listening` kanal icin yeniden baglanma politikasi.

        NEDEN AYRI BIR DONGU YOK: gateway'in kendi zamanlayicisiyla ikinci
        bir "uyandi mi" yoklama dongusu kurmak CAZIP ama YANLIS olurdu —
        opendnp3 kanali zaten baglanana kadar ustel geri cekilmeyle
        deniyor; ikinci dongu ayni sokete paralel SYN uretir ve iki
        zamanlayici birbirini yarisa sokar.

        VARSAYILAN OLCULDU (yadnp3 3.2.1.1): `ChannelRetry.Default()` =
        min 1000ms, max 60000ms ustel. Horstmann'in ~600sn'lik Socket
        Listening Timeout'u icinde >=10 deneme demektir; yani cihaz
        uyandiginda gateway onu kacirmadan yakalar. Bu yuzden varsayilan
        Smart icin de YETERLIDIR ve degistirilmesi ZORUNLU DEGILDIR.

        `smart_listen_reconnect_max_sec` verilirse yalnizca TAVAN kisilir:
        cihazin uyanisinin en gec ne kadar sonra fark edilecegini operator
        belirleyebilir. Taban 1sn'de kalir — yeni uyanmis bir cihaza hizli
        baglanmak istiyoruz.
        """
        # ARALIK VE TAM-TAMSAYI BURADA DA DOGRULANIR: disk onbelleginden gelen
        # `DeviceConfig` backend parser'ini ATLAR. Onbellekten gelen 0/negatif
        # ya da kesirli bir deger kutuphaneye gecerse yeniden baglanma
        # davranisi TANIMSIZ olurdu.
        tavan = exact_int(getattr(device, "smart_listen_reconnect_max_sec", None))
        if tavan is None:
            return opendnp3.ChannelRetry.Default()
        if not (SMART_LISTEN_RECONNECT_MIN_SEC <= tavan <= SMART_LISTEN_RECONNECT_MAX_SEC):
            return opendnp3.ChannelRetry.Default()
        return opendnp3.ChannelRetry(
            opendnp3.TimeDuration.Milliseconds(1000),
            opendnp3.TimeDuration.Seconds(tavan),
        )
    ...
    def enable_periodic_scans(self) -> bool:
        ...
        # SAHA TESHIS SATIRI: bu an itibariyla cihaz ARTIK SESSIZ DEGIL.
        # Master kurulum logu (`yadnp3_master_enabled`) acilis anini gosterir;
        # bu gecis SONRADAN olur ve onu gormeden "cihaz smart kosuyor"
        # sanilabilir. 2026-08-20 saha olayinda aranan bilgi tam da buydu.
        logger.info(
            "yadnp3_periodic_scans_enabled device=%s configured_policy=%s effective_policy=continuous "
            "periodic_scans=true event_scan=%ss baseline_scan=%ss — cihaz ARTIK 5sn'de bir "
            "Class 1/2/3 istegi gonderiyor; Horstmann hareketsizlik sayaci DOLAMAZ",
            self.device.code,
            self.configured_session_policy,
...
class Yadnp3TelemetryReader(TelemetryReader):
    ...
    def _init_runtime_state(
        ...
        *,
        ...
        `diagnostics=False`: arka plan tanilama havuzu KURULMAZ. Birim
        testleri gercek `ping` alt sureci baslatmasin diye; uretimde her
        zaman aciktir.
        ...
        #: Politika uyusmazligi uyarisi verilmis cihazlar (kenar-tetikli).
        self._politika_uyari_verilen: set[str] = set()
        ...
        #: SINIRLI arka plan tanilama havuzu. ICMP alt surec baslatir ve
        #: saniyelerce bloklayabilir; poll thread'inden DOGRUDAN cagrilmasi
        #: 200 gecikmis cihazda gateway'i dakikalarca durdururdu.
        #: Testler bunu `None` birakabilir -> sonda sessizce ATLANIR.
        self._tanilama_yurutucu: network_probe.DiagnosticExecutor | None = (
            network_probe.DiagnosticExecutor(name="dnp3-diag") if diagnostics else None
        )
    ...
    def _politika_uyusmazligini_bildir(self, mm: _ManagedMaster, device: DeviceConfig, gozlenen: str) -> None:
        ...
        POLITIKA DEGISTIRILMEZ. Bu yalnizca operatorun gormesi gereken bir
        yapilandirma uyusmazligidir ve 2026-08-20'de tam olarak bu bilgi
        eksikti: cihaz Smart moddayken gateway `continuous` kosuyordu ve
        5 saniyede bir Class 1/2/3 istegi gondererek Horstmann'in 600
        saniyelik hareketsizlik sayacinin dolmasini engelliyordu.
        """
        if gozlenen != MODE_SMART or mm.session_policy == "smart":
            return
        if device.code in self._politika_uyari_verilen:
            return
        self._politika_uyari_verilen.add(device.code)
        logger.warning(
            "device_policy_mismatch device=%s observed_operation_mode=smart "
            "configured_policy=%s effective_policy=%s periodic_scans=true — CIHAZ SMART "
            "MODDA ama gateway periyodik tarama gonderiyor; Horstmann hareketsizlik "
            "sayaci DOLAMAZ ve modem uyuyamaz. Gateway politikayi KENDILIGINDEN "
            "DEGISTIRMEZ: backend'de bu cihazin session_policy degerini 'smart' ya da "
            "'auto' yapin (bkz. docs/BACKEND_TODO.md#B5).",
            device.code,
            mm.configured_session_policy,
            mm.session_policy,
        )
    ...
    def _auto_politikayi_coz(
        ...
        *,
        ...
        GOZLEM ile EYLEM AYRI (2026-08-20 saha dersi)
        --------------------------------------------
        Mod HER cihazda OKUNUR ve raporlanir; POLITIKA ise YALNIZCA
        `configured == "auto"` iken degistirilir.

        Once ikisi de tek bir erken donuse baglanmisti. Sonucu sinsiydi:
        `continuous` yapilandirilmis bir cihaz mod noktasini durustce
        raporlasa bile gateway onu HIC yorumlamiyordu ve
        ...
        (log, /health, saglik basligi) GORUNMUYORDU — sahada teshis icin
        tcpdump acmak gerekti.

        BU BIR SEMANTIK DEGISIKLIGI DEGILDIR: gozlenen mod ETKIN POLITIKAYI
        `auto` DISINDA ASLA degistirmez. Grid'in yapilandirdigi politika
        otoriter KALIR (bkz. review §4).
        """
        ...
        if mm.configured_session_policy != "auto":
            # YALNIZCA GOZLEM — politikaya DOKUNULMAZ.
            if gozlenen != MODE_UNKNOWN:
                ...
                if gozlenen != mm.operation_mode:
                    ...
                    self._politika_uyusmazligini_bildir(mm, device, gozlenen)
            return False
        ...
        if not connected:
            ...
            # AYRICA: `auto_connected_since` YALNIZCA BAGLANTILI gecen sureyi
            # saymalidir. Uykuda gecen zaman fallback penceresini doldursaydi,
            # `auto` bir listening cihaz hicbir zaman siniflandirilamadan
            # `continuous`a duser ve tam da kacinmak istedigimiz surekli
            # trafigi uretirdi.
            mm.auto_connected_since = 0.0
            mm.auto_siniflama_pollu = False
            return False
        if mm.auto_connected_since == 0.0:
            mm.auto_connected_since = now
            # TEK ATISLIK SINIFLANDIRMA POLLU — YALNIZCA `listening`.
            #
            # `auto` + mod bilinmiyor -> master SESSIZ kurulur (tarama yok,
            # acilis integrity yok). Iyi bir varsayilan, ama `listening` ucta
            # bir cikmaz uretir: gateway hicbir sey sormazsa `Operation Mode`
            # noktasi HIC okunmaz, mod hicbir zaman ogrenilmez ve cihaz 120sn
            # sonra `continuous`a duser — Smart bir cihaz icin YANLIS SONUC.
            #
            # `listening`te baglanti KURULMUS olmasi cihazin UYANIK oldugunun
            # kanitidir; tek bir integrity poll onun 15sn'lik hareketsizlik
            # sayacini BIR KEZ sifirlar, sonra gateway yeniden susar. Maliyet
            # bir alisveris, kazanc dogru siniflandirma.
            #
            # `initiating` BILEREK DISARIDA: orada cihaz kendi raporunu
            # gonderiyorken araya soru sokmak 1.13.0'da dogrulanmis davranisi
            # degistirirdi. O yol AYNEN korunur.
            uc = str(getattr(device, "ip_endpoint_type", "") or "listening").strip().lower()
            if uc != "initiating" and not mm.auto_siniflama_pollu:
                mm.auto_siniflama_pollu = True
                if mm.request_integrity_poll():
                    logger.info(
                        "auto_classify_poll device=%s endpoint=listening — mod "
                        "bilinmiyor; oturum basina TEK integrity poll gonderildi "
                        "(sonrasinda gateway yeniden susar)",
                        device.code,
                    )
            return False
    ...
    def _session_policy(device: DeviceConfig) -> str:
        ...
        UC TIPI ILE MOD BAGIMSIZ IKI KAVRAMDIR (1.14.0)
        -----------------------------------------------
        1.13.0'da `smart`/`auto` YALNIZCA `initiating` ile gecerliydi ve
        `listening` uc sessizce `continuous`a dusuruluyordu. Bu KISIT
        KALDIRILDI cunku YANLISTI: uc tipi baglantiyi KIMIN acacagini
        soyler, Operation Mode ise cihazin MODEMINI kapatip kapatmadigini.
        Alti kombinasyonun HEPSI sahada gecerlidir:

            listening + boost   initiating + boost
            listening + smart   initiating + smart
            listening + auto    initiating + auto

        `listening` + `smart` gerçek ve YAYGIN bir kurulumdur: sabit IP'li
        (ya da APN icinden erisilebilen) bir Horstmann, Smart modda
        modemini kapatir. Gateway'in dogru davranisi, o cihaza SUREKLI
        SYN gondermek DEGIL, uykuyu KABUL EDIP (`smart_idle`) sessizlik ve
        Dial-In denetimlerine birakmaktir — bkz. `_smart_idle_denetle`.

        Eski dusurmenin zarari somuttu: Smart bir listening cihaz
        `continuous` calistirilinca gateway her tarama araliginda frame
        gonderir, cihazin 15 saniyelik hareketsizlik sayaci HIC dolmaz ve
        modem HICBIR ZAMAN kapanmaz — yani pil/veri tasarrufu tamamen
        kaybolur.
    ...
    @staticmethod
    def _dial_in_limit_sn(device: DeviceConfig) -> int | None:
        """Beklenen rapor araligi (saniye); None ise `late` HIC URETILMEZ.

        Alan opsiyoneldir ve olmamasi bir ariza degildir: her Horstmann
        kurulumunda zamanlanmis Dial-In tanimli olmayabilir. Tanimli
        degilse gecikme diye bir kavram da yoktur ve tek denetim
        `smart_max_silence_sec` olarak kalir.

        ARALIK BURADA DA DOGRULANIR — `_smart_silence_limit` ile ayni gerekce:
        `DeviceConfig` iki yoldan gelir ve DISK ONBELLEGI yolu parser'i ATLAR
        (`state.load_from_cache` alanlari dogrudan JSON'dan kurar). Onbellekten
        gelen 1 dakikalik bir deger her uyuyan cihazi kalici olarak `late`
        gosterir ve uyariyi anlamsizlastirirdi.
        """
        dakika = exact_int(getattr(device, "dial_in_interval_min", None))
        if dakika is None:
            return None
        if not (DIAL_IN_INTERVAL_MIN_MIN <= dakika <= DIAL_IN_INTERVAL_MIN_MAX):
            return None
        return dakika * 60

    @staticmethod
    def _sessizlik_yasi(mm: _ManagedMaster) -> float:
        """Son GECERLI temastan bu yana gecen sure (monotonic, saniye).

        Hic temas yoksa master'in kurulus anindan olculur. Bu capa ZORUNLU:
        listening+smart bir cihaz ilk gecerli temasi HIC yapmadan idle'a
        girebilir ve capasiz bir sessizlik denetimi o cihaz icin hicbir zaman
        baslamazdi — yani gercekten olu bir cihaz sonsuza kadar "uyuyor"
        gorunurdu.
        """
        son = mm.cache.last_valid_contact()
        if son:
            return time.monotonic() - son
        return max(0.0, time.time() - mm.olusturuldu_wall)

    def _sessizlik_asildi(self, mm: _ManagedMaster, device: DeviceConfig) -> bool:
        """Izin verilen sessizlik penceresi asildi mi (esik yoksa asla)."""
        limit = self._smart_silence_limit(device)
        return bool(limit) and self._sessizlik_yasi(mm) > limit

    def _rapor_gecikmesi(self, mm: _ManagedMaster, device: DeviceConfig) -> tuple[float | None, bool]:
        """(gecikme_saniye, gecikti_mi) — beklenen Dial-In penceresine gore.

        Sure MONOTONIC olculur; duvar saatiyle olculse tek bir NTP siçramasi
        tum uyuyan filoyu ayni anda "gecikmis" gosterirdi.

        Referans ani: son GECERLI TEMAS. Hic temas yoksa master'in kurulus
        ani kullanilir — aksi halde hic konusmamis bir cihaz icin gecikme
        hesabi hicbir zaman baslamazdi.
        """
        limit = self._dial_in_limit_sn(device)
        if not limit:
            return None, False
        yas = self._sessizlik_yasi(mm)
        return yas - limit, yas > limit

    def _smart_idle_denetle(
        self, mm: _ManagedMaster, device: DeviceConfig, *, connected: bool = False
    ) -> bool:
        ...
        --------------------------
        Orada baglantiyi GATEWAY acar. Cihaz uykudayken TCP baglantisi
        KURULAMAZ, dolayisiyla `OnOpen`/`OnClose` HIC tetiklenmez ve
        `set_connected(False)` yolundan idle'a GIRILEMEZ. Bu dal olmadan
        Smart bir listening cihaz `lost`ta kalir ve HER uyku doneminde sahte
        comm_lost uretirdi — yani ozelligin tamami calismazdi.
        """
        cache = mm.cache

        # LATE takibi HER CYCLE'DA, hangi dala gidilirse gidilsin. Cihaz
        # uyanip `online` olunca bayragin temizlenmesi buna bagli.
        self._rapor_gecikmesini_izle(mm, device)
        ...
        if not cache.is_smart_idle():
            # LISTENING + SMART: baglanti YOK -> BEKLENEN uyku.
            #
            # Kanit sarti ARANMAZ (initiating'in aksine): orada cihazin
            # baglanip hic konusmamasi bir ariza belirtisidir, burada ise
            # baglanti KURULAMAMASI zaten uykunun TANIMIDIR. Basarisiz TCP
            # denemeleri tek baslarina comm_lost URETMEZ.
            #
            # Bu KORUMASIZ bir af DEGILDIR: asagidaki sessizlik denetimi
            # `olusturuldu_wall` capasi sayesinde hic konusmamis bir cihaz
            # icin de isler; gercekten olu bir cihaz `smart_max_silence_sec`
            # dolunca yine `lost` olur.
            uc = (device.ip_endpoint_type or "listening").strip().lower()
            if uc == "initiating" or connected:
                return False
            # SESSIZLIK ZATEN ASILMISSA GIRME. Bu satir olmadan durum
            # SALINIRDI: `smart_idle_to_lost` cihazi `lost` yapar, bir
            # sonraki cycle burasi ayni kosulla onu tekrar `smart_idle`e
            # sokardi ve comm_lost TEK CYCLE sonra susardi — gercekten
            # kaybolmus bir cihaz SCADA'da canli gorunmeye devam ederdi.
            # Kosul kendi kendini duzeltir: taze bir temas gelince
            # `last_valid_contact` tazelenir ve girise yeniden izin verilir.
            if self._sessizlik_asildi(mm, device):
                return False
            if cache.enter_smart_idle(anchor_wall=mm.olusturuldu_wall):
                with self._smart_sayaclari_lock:
                    self._smart_idle_giris_total += 1
                logger.info(
                    "smart_idle_entered device=%s ip=%s reason=listening_unreachable — "
                    "cihaza baglanilamiyor; Smart uykusu KABUL EDILDI, comm_lost "
                    "YAYINLANMADI (denetim: dial_in + max_silence)",
                    device.code,
                    device.ip_address,
                )
        ...
        limit = self._smart_silence_limit(device)
        if limit:
            # Yas `_sessizlik_yasi` uzerinden: hic gecerli temas yapmamis bir
            # listening+smart cihaz icin de denetim ISLER (capa = master'in
            # kurulus ani). Eski hali orada `None` uretip denetimi tamamen
            # atliyordu.
            yas = self._sessizlik_yasi(mm)
            if yas > limit:
                ...
                mm.rapor_gecikti = False
                return False

        # Sonda YALNIZCA gecikmisken calisir: normal uykuda ping/TCP denemesi
        # gereksiz trafik ve gurultu olurdu.
        if mm.rapor_gecikti:
            self._sonda_calistir(mm, device)
        return True

    def _rapor_gecikmesini_izle(self, mm: _ManagedMaster, device: DeviceConfig) -> None:
        """LATE bayragini tazele (kenar-tetikli log). comm_lost URETMEZ.

        Uc ayri pencere vardir ve KARISTIRILMAMALIDIR:
            beklenen rapordan ONCE          -> smart_idle, SAGLIKLI
            rapor gecti ama max_silence YOK -> LATE, DEGRADED (kopuk DEGIL)
            max_silence asildi              -> LOST, comm_lost TAM BIR KEZ

        LATE'in tek islevi operatore erken uyari vermek ve aktif tanilama
        sondasini tetiklemektir; durum makinesine DOKUNMAZ.

        HER CYCLE'DA, IDLE OLSUN OLMASIN CAGRILIR. Yalnizca idle dalinda
        cagrilsaydi bayrak cihaz uyanip `online` olduktan sonra TEMIZLENMEZ
        ve cihaz saglikliyken sonsuza kadar "gec kalmis" gorunurdu.
        """
        gecikme, gecikti = self._rapor_gecikmesi(mm, device)
        if gecikti and not mm.rapor_gecikti:
            mm.rapor_gecikti = True
            logger.warning(
                "smart_report_overdue device=%s ip=%s overdue=%.0fs dial_in=%smin — "
                "beklenen Dial-In raporu GELMEDI; cihaz LATE (degraded) sayiliyor. "
                "comm_lost URETILMEDI, tanilama sondasi calisacak",
                device.code,
                device.ip_address,
                gecikme or 0.0,
                getattr(device, "dial_in_interval_min", None),
            )
        elif not gecikti and mm.rapor_gecikti:
            mm.rapor_gecikti = False
            logger.info(
                "smart_report_recovered device=%s — rapor penceresi normale dondu",
                device.code,
            )

    # ---- Aktif tanilama sondalari — SAGLIK KARARI VERMEZLER --------------

    def _sonda_calistir(self, mm: _ManagedMaster, device: DeviceConfig) -> None:
        """ICMP tanilamasini ARKA PLANA KUYRUKLA. ANINDA doner, BLOKLAMAZ.

        IKI IHLAL EDILEMEZ KURAL
        ------------------------
        1. Hicbir sonuc `comm_lost` URETMEZ.
        2. Bu cagri DNP3 okuma/poll yolunu BLOKLAMAZ.

        NEDEN SENKRON OLAMAZ — SURU ETKISI
        ----------------------------------
        Cihaz basina 300sn'lik siklik siniri TEK BASINA YETMEZ. 200 cihaz ayni
        anda `late` olursa (APN kesintisi, saha elektrigi — gercek senaryolar)
        her biri ilk sondasini AYNI cycle'da hak eder ve senkron cagrilarda
        bunlar poll thread'inde SIRAYA girer:

            200 cihaz x ~2sn ICMP zaman asimi = ~400 saniye

        Bu sure boyunca gateway HICBIR cihazi okuyamaz — yani tanilama, teshis
        etmeye calistigi kesintiyi GERCEK bir kesintiye cevirir.

        TCP TESHISI BURADA YAPILMAZ: yadnp3 kanal durumundan PASIF olarak
        turetilir (`_make_channel_listener`). Cihazin DNP3 portuna ham soket
        acmak uretim kanaliyla YARISIRDI (bkz. `network_probe` modul basligi).

        Siklik `_PROBE_MIN_INTERVAL_SEC` ile sinirlidir; poll dongusu saniyede
        bir kosar ve her turda ping baslatmak hem gurultu hem yuk olurdu.
        """
        simdi = time.monotonic()
        if mm.sonda_son_at and (simdi - mm.sonda_son_at) < _PROBE_MIN_INTERVAL_SEC:
            return
        # Damga KUYRUKLAMADAN once atilir: is dusurulse bile siklik siniri
        # ilerlemeli, aksi halde dolu kuyruk her cycle'da yeniden denenir.
        mm.sonda_son_at = simdi

        yurutucu = self._tanilama_yurutucu
        if yurutucu is None:
            return
        host = (device.ip_address or "").strip()
        kod = device.code

        def _is() -> None:
            # ARKA PLAN THREAD'I. Buradaki hicbir sey durum makinesine
            # DOKUNMAZ; yalnizca teshis alanlarini tazeler.
            ip_sonuc = network_probe.icmp_probe(host)
            mm.ip_probe = ip_sonuc
            mm.sonda_son_wall = time.time()
            tcp_sonuc = mm.kanal_durumu()
            logger.info(
                "device_probe device=%s ip=%s ip_probe=%s tcp_state=%s — %s",
                kod,
                host,
                ip_sonuc,
                tcp_sonuc,
                network_probe.diagnose(ip_sonuc, tcp_sonuc, self._dnp3_kaniti(mm)),
            )

        yurutucu.submit(kod, _is)

    @staticmethod
    def _dnp3_kaniti(mm: _ManagedMaster) -> str:
        """DNP3 oturum kaniti — durum makinesinden TURETILIR, olculmez."""
        durum = mm.cache.state()
        if durum == "online":
            return network_probe.DNP3_HEALTHY
        if durum in ("lost", "recovering"):
            return network_probe.DNP3_FAILED
        return network_probe.DNP3_UNKNOWN
    ...
    def _oku(
        ...
        *,
        ...
        # SMART'ta OTURUM BASINA TEK DENEME. Backoff yeniden denemeleri
        # (15/30/60/120/240 sn) tam da sessizlik penceresinin ortasina duser
        # ve Horstmann'in hareketsizlik sayacini sifirlar — yani modem
        # uyuyamaz. Bu yol, ayni fonksiyondaki diger TUM yoklamalarin aksine
        # (`sessiz_yoklama_serbest`) korumasizdi.
        #
        # TEK deneme bilincli olarak KORUNUYOR: seri no / IMEI / firmware
        # bilgisi operator icin degerlidir ve tek atislik acilis isi
        # sozlesmece serbesttir. Cihaz cevap vermezse bir daha sorulmaz.
        if connected and cache.g110_gerekli(max_deneme=1 if smart else None):
            mm.scan_g110_once()
        ...
        if smart and self._smart_idle_denetle(mm, device, connected=connected):
        ...
        #
        # SMART'TA BU KURAL UYGULANMAZ (2026-08-20 saha hatasi).
        # ----------------------------------------------------
        # Bayatlik esigi `scan_interval`/`baseline_interval`den turetilir ve
        # ortuk varsayimi sudur: "saglikli cihaz her tarama turunda cevap
        # verir". Bu varsayim SUREKLI politikaya aittir. Smart'ta HICBIR
        # tarama kurulmaz, dolayisiyla kanit da gelmez — yani KANITLANMIS,
        # SAGLIKLI bir Smart oturumu 120 saniye sonra GARANTILI olarak
        # "bayat" olur ve sahte comm_lost uretir. Sahada tam bu gorulduu:
        #     13:09:16 device_recovered  ->  13:11:16 device_stale (120s)
        #                                    13:11:16 comm_lost_announced
        # Cihaz o sirada TCP'de canli ve DNP3'te saglikliydi.
        #
        # Smart bir oturum `online`dan YALNIZCA su gercek olaylarla cikar:
        #   * link kapanir            -> smart_idle (kanit varsa) / lost
        #   * `smart_max_silence_sec` -> lost (gercek kopus denetimi)
        # Uzun vadeli "cihaz kayip mi" olcutu Dial-In + sessizlik esigidir,
        # olcum yasi DEGILDIR.
        if connected and stale and cache_state == "online" and not smart:
            cache.begin_recovery()
        ...
        if cache_state == "lost" and connected and not stale and cache.relink_izinli():
            cache.begin_recovery()
            cache_state = "recovering"
            logger.info(
        ...
        if cache_state != "online":
            # SMART UYANMA PENCERESI — comm_lost ERTELENIR, bastirilmaz.
            # ---------------------------------------------------------
            # Smart bir cihaz uyandiginda `OnOpen` -> `begin_recovery()` olur
            # ve durum bir sure `recovering` kalir. Cihaz TCP el sikismasini
            # bitirip ilk DNP3 fragmentini gonderene kadar gecen bu KISA
            # pazarlik suresinde comm_lost yayinlamak SAHTE ALARMDIR: cihaz
            # uykuya girmeden ONCE saglikliydi ve su an fiilen konusuyor.
            #
            # Sahada gorulduu (2026-08-20):
            ...
            #
            # ERTELEME BASTIRMA DEGILDIR: grace (15sn) GERCEKTEN dolarsa
            # `fail_recovery` durumu `lost` yapar ve comm_lost bir sonraki
            # cycle'da YAYINLANIR. En fazla grace kadar gecikir.
            #
            # `continuous`ta DOKUNULMAZ: orada `recovering` sirasinda
            # comm_lost tutmak bilincli bir karardir (sahte "online"
            # gostermemek icin) ve uzun sureli uretim davranisidir.
            if smart and cache_state == "recovering" and cache.comm_lost_ertelenebilir(_RECOVERY_GRACE_SEC):
                return [
                    SignalReading(
                        signal_key=s.key,
                        source=s.source,
    ...
    def device_health(self) -> dict[str, dict[str, Any]]:
        ...
        for code, mm in masters:
            try:
                ...
                # DIAL-IN PENCERESI — "ne zaman haber bekliyoruz" sorusunun
                # cevabi. `smart_silence_*` ile KARISTIRILMAMALI: o kopus
                # esigidir, bu ise beklenen rapor ani. Ikisi arasindaki
                # bolgede cihaz LATE'tir, kopuk DEGIL.
                dial_in = self._dial_in_limit_sn(mm.device)
                sonraki_rapor: float | None = None
                gecikme_sn: float | None = None
                if dial_in:
                    capa = son_temas_wall or mm.olusturuldu_wall
                    sonraki_rapor = capa + dial_in
                ...
                out[code] = {
                    ...
                    # --- Dial-In farkindali saglik (LATE) ------------------
                    # `report_late=true` DEGRADED demektir, KOPUK DEGIL.
                    # `state` bu yuzden `smart_idle` kalir: durum makinesi
                    # sondalardan ve gecikmeden ETKILENMEZ.
                    "dial_in_interval_min": getattr(mm.device, "dial_in_interval_min", None),
                    "next_expected_report_epoch": sonraki_rapor,
                    "report_overdue_sec": gecikme_sn,
                    "report_late": bool(mm.rapor_gecikti),
                    # --- Tanilama -----------------------------------------
                    # SALT TESHIS. Bu uc alandan HICBIRI comm_lost uretmez ve
                    # hicbiri `state` hesabina girmez (bkz. network_probe).
                    #
                    # `ip_probe_status` ARKA PLANDA olculur (ICMP); `/health`
                    # onun bitmesini BEKLEMEZ, son bilinen degeri doner.
                    # `tcp_probe_status` yadnp3 KANAL DURUMUNDAN turetilir —
                    # cihazin DNP3 portuna ham soket ACILMAZ (uretim kanaliyla
                    # yarisirdi).
